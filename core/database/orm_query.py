...
from core.database.models import (Base, User, Portfolio, 
                                  Transaction, Coin, Timeseries,
                                  News, Agent, NewsModel, RiskModel,
                                  MMM)


async def orm_update_coin_list(session: AsyncSession, coin_list: list[str]):
    query = select(Coin)
    result = await session.execute(query)
    for coin in result.scalars().all():
        if coin.name not in coin_list:
            session.delete(coin)
        else:
            coin_list.remove(coin.name)

    if coin_list:
        session.add_all([Coin(name=name) for name in coin_list])

    await session.commit()

async def orm_get_coin_list(session: AsyncSession):
    query = select(Coin).options(joinedload(Coin.timeseries))
    result = await session.execute(query)
    return result.scalars().all()

async def orm_update_coin_price(session: AsyncSession, coin: str, price: float):
    query = update(Coin).where(Coin.name == coin).values(price_now=price)
    await session.execute(query)
    await session.commit()

async def orm_get_coin(session: AsyncSession, coin: str) -> Coin:
    query = select(Coin).where(Coin.name == coin).options(joinedload(Coin.timeseries))
    result = await session.execute(query)

    return result.scalar()

async def orm_add_timeseries(session: AsyncSession, coin: str, timestamp:str, path: str):
    
    coin = await orm_get_coin(session, coin)
    
    if coin:
        session.add(Timeseries(coin=coin, timestamp=timestamp, path_dataset=path))
        await session.commit()
        return True
...
async def orm_update_timeseries_path(session: AsyncSession, coin: str, timestamp:str, path: str):
    query = update(Timeseries).where(Timeseries.coin == coin, Timeseries.timestamp == timestamp).values(path_dataset=path)
    await session.execute(query)
    await session.commit()
    
async def orm_get_timeseries(session: AsyncSession, coin: str, timestamp:str=None) -> list[Timeseries]:
    coin: Coin = await orm_get_coin(session, coin)

    if coin:
        if timestamp:
            query = select(Timeseries).where(Timeseries.coin == coin, Timeseries.timestamp == timestamp)
            result = await session.execute(query)
            return result.scalars().all()
        
        return coin.timeseries
    else:
        raise ValueError("Coin not found")
